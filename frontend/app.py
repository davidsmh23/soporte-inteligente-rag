...
user_token = st.sidebar.text_input(
    "User Token", value=default_user_token, type="password"
)
...
def _refresh_conversations():
    res = _request_json(
        "GET", f"{GATEWAY_URL}/api/v1/conversations", params={"user_id": user_id}
    )
...
def _load_conversation(conversation_ref: str):
    ...
    st.session_state.messages_by_conversation[conversation_id] = messages or [
        _welcome_message()
    ]
    ...
    if st.session_state.conversations:
        ...
        if conv_id and conv_id not in st.session_state.messages_by_conversation:
            try:
                ...
                st.session_state.messages_by_conversation[conv_id] = [
                    _welcome_message()
                ]
        return
...
try:
    ...
    if status.status_code == 200:
        ...
        if status_json.get("connected"):
            label = (
                "Conectado y ocupado"
                if status_json.get("busy")
                else "Conectado y libre"
            )
...
with left_col:
    ...
    current_mode = st.session_state.chat_mode_by_conversation.get(
        active_conversation_id, "chat_generico"
    )
    ...
    if active_conversation_id not in st.session_state.messages_by_conversation:
        st.session_state.messages_by_conversation[active_conversation_id] = [
            _welcome_message()
        ]
...
if user_input and st.session_state.active_conversation_id:
    active_conversation_id = st.session_state.active_conversation_id
    active_codex_session_id = st.session_state.active_codex_session_id
    messages = st.session_state.messages_by_conversation.get(
        active_conversation_id, [_welcome_message()]
    )
    messages.append(
        {"role": "user", "content": user_input, "references": [], "token_usage": None}
    )
    st.session_state.messages_by_conversation[active_conversation_id] = messages
                ...
                with requests.post(
                    ...
                    for raw_line in response.iter_lines(decode_unicode=True):
                        ...
                        if event_type == "conversation_bound":
                            ...
                            bound_codex = (
                                str(event.get("codex_session_id") or "").strip() or None
                            )
                        ...
                        if event_type == "final_answer":
                            bound_codex_in_final = (
                                str(event.get("codex_session_id") or "").strip() or None
                            )
                            if (
                                bound_codex_in_final
                                and not st.session_state.active_codex_session_id
                            ):
                                st.session_state.active_codex_session_id = (
                                    bound_codex_in_final
                                )
